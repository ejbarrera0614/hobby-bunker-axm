...
export function QuestAddPage (): JSX.Element {
  ...
  return (
      <form onSubmit={onSubmit} className='flex flex-col gap-6 max-w-md '>
        <h1>{t('addQuest')}</h1>
        <SelectListGameType label='Selecciona un tipo de juego' className="bg-white w-full max-w-xl" {...register('GameType', useErrors({required: true}) )} error={errors?.GameType}/>
